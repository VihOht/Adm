...
function updateFilterSummary() {
    ...
    if (currentFilters.expenseCategory) {
        ...
        if (currentFilters.expenseCategory !== 'no-category') {
            // Look for category name in the category dropdown options in the template
            const expenseCategoryOptions = document.querySelectorAll('#expense-category-filter option, #advanced-expense-category option');
            for (const option of expenseCategoryOptions) {
                if (option.value === currentFilters.expenseCategory) {
                    categoryText = option.textContent;
                    break;
                }
            }
            
            // If still not found, look in the category buttons from the dropdown filter
            if (categoryText === 'Sem categoria') {
                const categoryButtons = document.querySelectorAll('#expense-categories-list .category-option');
                for (const button of categoryButtons) {
                    const onclick = button.getAttribute('onclick');
                    if (onclick && onclick.includes(`'${currentFilters.expenseCategory}'`)) {
                        categoryText = button.textContent.trim();
                        break;
                    }
                }
            }
        }
        activeFilters.push(`Gasto: ${categoryText}`);
    }
    ...
    if (currentFilters.incomeCategory) {
        ...
        if (currentFilters.incomeCategory !== 'no-category') {
            // Look for category name in the category dropdown options in the template
            const incomeCategoryOptions = document.querySelectorAll('#income-category-filter option, #advanced-income-category option');
            for (const option of incomeCategoryOptions) {
                if (option.value === currentFilters.incomeCategory) {
                    categoryText = option.textContent;
                    break;
                }
            }
            ...
            // If still not found, look in the category buttons from the dropdown filter
            if (categoryText === 'Sem categoria') {
                const categoryButtons = document.querySelectorAll('#income-categories-list .category-option');
                for (const button of categoryButtons) {
                    const onclick = button.getAttribute('onclick');
                    if (onclick && onclick.includes(`'${currentFilters.incomeCategory}'`)) {
                        categoryText = button.textContent.trim();
                        break;
                    }
                }
        ...
        activeFilters.push(`Receita: ${categoryText}`);
    }
    
    if (currentFilters.dateRange.start || currentFilters.dateRange.end) {
        let rangeText = 'Período: ';
        if (currentFilters.dateRange.start && currentFilters.dateRange.end) {
            const startDate = new Date(currentFilters.dateRange.start).toLocaleDateString('pt-BR');
            const endDate = new Date(currentFilters.dateRange.end).toLocaleDateString('pt-BR');
            rangeText += `${startDate} até ${endDate}`;
        } else if (currentFilters.dateRange.start) {
            const startDate = new Date(currentFilters.dateRange.start).toLocaleDateString('pt-BR');
            rangeText += `a partir de ${startDate}`;
        } else {
            const endDate = new Date(currentFilters.dateRange.end).toLocaleDateString('pt-BR');
            rangeText += `até ${endDate}`;
        }
